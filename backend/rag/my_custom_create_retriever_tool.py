...
class MyRetrieverInput(BaseModel):
...
def create_retriever_tool(
    ...
    *,
    ...
    def _get_relevant_documents(
        ...
        # 直接使用外部函数的参数 retriever, document_prompt 等
        docs = retriever.invoke(query, config={"callbacks": callbacks})
        content = document_separator.join(
            format_document(doc, document_prompt) for doc in docs
        )
        if response_format == "content_and_artifact":
            return content, list(dict.fromkeys([doc.metadata['filename'] for doc in docs]))
        return content
    ...
    return Tool(
        ...
        coroutine=_aget_relevant_documents,
        args_schema=MyRetrieverInput,
